...
import { PinConfig, PinStyle, AspectRatio, ImageModel, ContentType, ImageSize, LogoPosition } from '@/lib/types';
import { Upload, Link2, Image, X, Plus, Sparkles, Loader2 } from 'lucide-react';
...
export default function InputSection({ onGeneratePrompts, isProcessing, config, onConfigChange }: InputSectionProps) {
    ...
    return (
        ...
        >
            {/* Content Type Toggle */}
            <div className="mb-4 p-1 rounded-lg flex" style={{ background: '#0F172A' }}>
                <button onClick={() => onConfigChange({ ...config, contentType: 'article' })}
                    className="flex-1 py-1.5 text-xs font-semibold rounded-md transition-all"
                    style={config.contentType === 'article' ? { background: '#334155', color: 'var(--primary)' } : { color: '#94A3B8' }}>
                    Standard Post
                </button>
                <button onClick={() => onConfigChange({ ...config, contentType: 'product' })}
                    className="flex-1 py-1.5 text-xs font-semibold rounded-md transition-all"
                    style={config.contentType === 'product' ? { background: '#334155', color: 'var(--primary)' } : { color: '#94A3B8' }}>
                    Product
                </button>
            </div>
            ...
            <textarea
                className="w-full min-h-[400px] rounded-lg p-3 text-xs font-mono outline-none resize-y mb-4 focus:ring-2 focus:ring-yellow-400"
            ...
            <div className="mb-4 pt-4" style={{ borderTop: '1px solid #334155' }}>
                ...
                <div className="rounded-lg p-3 space-y-3" style={{ background: '#0F172A', border: '1px solid #334155' }}>
                    {!config.logoData ? (
                        <div onClick={() => logoInputRef.current?.click()}
                            className="w-full h-12 border border-dashed rounded flex items-center justify-center cursor-pointer text-[10px]"
                            style={{ borderColor: '#475569', color: '#94A3B8' }}>
                            + Upload Logo (PNG)
                        </div>
                    ) : (
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded flex items-center justify-center p-1" style={{ border: '1px solid #475569', background: '#1E293B' }}>
                                <img src={config.logoData} alt="Logo" className="max-w-full max-h-full object-contain" />
                            </div>
                            <span className="text-[10px]" style={{ color: '#94A3B8' }}>Logo loaded</span>
                        </div>
                    )}
                    ...
                    {config.logoData && (
                        <>
                            <div>
                                <label className="block text-[10px] font-semibold mb-1" style={{ color: '#94A3B8' }}>Position</label>
                                <select value={config.logoPosition || 'bottom-right'} onChange={(e) => onConfigChange({ ...config, logoPosition: e.target.value as LogoPosition })}
                                    className="w-full text-xs rounded p-1.5" style={{ background: '#1E293B', border: '1px solid #475569', color: '#E2E8F0' }}>
                                    {positionOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                                </select>
                            </div>
                            <div>
                                <div className="flex justify-between items-center mb-1">
                                    <label className="block text-[10px] font-semibold" style={{ color: '#94A3B8' }}>Size</label>
                                ...
                                <input type="range" min="10" max="60" step="5" value={config.logoSize || 20}
                                    ...
                                    className="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-pink-500" style={{ background: '#475569' }} />
                            </div>
                        </>
                    )}
                </div>
            ...
            <div className="mb-4 pt-4" style={{ borderTop: '1px solid #334155' }}>
                ...
                <div className="rounded-lg p-3 space-y-3" style={{ background: '#0F172A', border: '1px solid #334155' }}>
                    <div>
                        <label className="block text-[10px] font-semibold mb-1" style={{ color: '#94A3B8' }}>Button Text</label>
                        <input type="text" value={config.ctaText || ''} onChange={(e) => onConfigChange({ ...config, ctaText: e.target.value })}
                            placeholder="e.g. Shop Now, Read More" className="w-full text-xs rounded p-2" style={{ background: '#1E293B', border: '1px solid #475569', color: '#E2E8F0' }} />
                    </div>
                    {config.ctaText && (
                        <>
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    <label className="block text-[10px] font-semibold mb-1" style={{ color: '#94A3B8' }}>Button Color</label>
                                ...
                                <div className="flex-1">
                                    <label className="block text-[10px] font-semibold mb-1" style={{ color: '#94A3B8' }}>Text Color</label>
                                    <div className="flex gap-2 items-center">
            ...
            <div className="mb-4 pt-4" style={{ borderTop: '1px solid #334155' }}>
                <label className="text-xs font-bold uppercase tracking-wide mb-2 block" style={{ color: '#E2E8F0' }}>
                    Reference Assets <span className="font-normal normal-case" style={{ color: '#64748B' }}>(Optional)</span>
                </label>
                <div className="flex gap-2 mb-3">
                    <input type="text" value={imageUrlInput} onChange={(e) => setImageUrlInput(e.target.value)} placeholder="Paste Image Link..."
                        className="flex-1 rounded-lg px-2 py-1.5 text-xs" style={{ background: '#0F172A', border: '1px solid #334155', color: '#E2E8F0' }} />
                    <button onClick={handleAddImageUrl} disabled={isLoadingUrl || !imageUrlInput}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background: '#334155', color: 'var(--primary)', border: '1px solid #475569' }}>
                        {isLoadingUrl ? '...' : 'Add'}
                    </button>
                </div>
                ...
                <div onClick={() => refImageInputRef.current?.click()}
                    className="w-full h-14 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer mb-3"
                ...
                {(config.referenceImages?.length || 0) > 0 && (
                    <div className="grid grid-cols-3 gap-2">
                        {config.referenceImages?.map((img, idx) => (
                            <div key={idx} className="relative aspect-square rounded overflow-hidden group" style={{ background: '#0F172A', border: '1px solid #334155' }}>
                                <img src={img} alt={`Ref ${idx}`} className="w-full h-full object-cover" />
                                <button onClick={() => removeReferenceImage(idx)}
                                    className="absolute top-1 right-1 bg-black/60 hover:bg-black/80 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100">
                                    <X className="w-3 h-3" />
                                </button>
                            </div>
                        ))}
            ...
            {/* Pin Settings */}
            <div className="space-y-4 flex-1 pt-4" style={{ borderTop: '1px solid #334155' }}>
                <div>
                    ...
                    <select value={config.style} onChange={(e) => onConfigChange({ ...config, style: e.target.value as PinStyle })}
                        className="w-full text-xs rounded-lg p-2.5" style={{ background: '#0F172A', border: '1px solid #334155', color: '#E2E8F0' }} disabled={isProcessing}>
                ...
                <div>
                    ...
                    <input type="text" value={config.websiteUrl || ''} onChange={(e) => onConfigChange({ ...config, websiteUrl: e.target.value })}
                        placeholder="e.g. mydomain.com" className="w-full rounded-lg p-2.5 text-xs" style={{ background: '#0F172A', border: '1px solid #334155', color: '#E2E8F0' }} disabled={isProcessing} />
                </div>
            </div>
